...
DEFAULT_PROMPTS = {
    ...
    PROMPT_CHAT: (
        ...
        '只依据提供的文献题名、摘要、关键词和图谱节点作答，不要编造实验数据。'
        '若文献不足以回答，明确说明缺口，并列出最相关的论文。'
    ...
    def _seed_prompts(self) -> None:
        ...
        for name, body in DEFAULT_PROMPTS.items():
            exists = self.conn.execute(
                'SELECT name FROM kg_prompts WHERE name = ?',
                (name,),
            ).fetchone()
            if exists:
                continue
    ...
    def graph_payload(self, run_id: int) -> dict[str, Any]:
        ...
        nodes = [
            ...
            for row in self.conn.execute(
                'SELECT node_key, label, node_type, properties_json, source FROM kg_nodes WHERE run_id = ?',
                (run_id,),
            ).fetchall()
        ]
        edges = [
            dict(row)
            for row in self.conn.execute(
                'SELECT source_key, target_key, rel_type, properties_json FROM kg_edges WHERE run_id = ?',
                (run_id,),
            ).fetchall()
        ]
        for node in nodes:
        ...
        for node in graph.get('nodes') or []:
            ...
            self.conn.execute(
                ...
                INSERT INTO kg_nodes (
                    run_id, document_id, node_key, label, node_type, properties_json, source
                ) VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    run_id,
        ...
        for edge in graph.get('edges') or []:
            ...
            self.conn.execute(
                ...
                INSERT INTO kg_edges (
                    run_id, document_id, source_key, target_key, rel_type, properties_json
                ) VALUES (?, ?, ?, ?, ?, ?)
                """,
                (
                    run_id,
...
def system_graph(document: dict[str, Any]) -> dict[str, Any]:
    ...
    if not names:
        names = _keywords(document.get('title') or '', document.get('description') or '')
        source = 'heuristic'
    else:
        source = 'bound'
...
def resolve_extract_prompt(
    ...
    for item in keywords or []:
        ...
        parts.append(f'## Keyword: {heading}\n{body}')
        used.append(name or heading)
    if not parts:
        return default, PROMPT_EXTRACT
    suffix = (
        '\n\nAlways keep file/system attributes (time, institution, authors, filename, abstract). '
        'Return JSON only with nodes and edges.'
    )
    label = 'keyword:' + ','.join(used[:4]) if used else 'keyword:custom'
